...
class FeedController(private val feedService: FeedService,  private val objectMapper: ObjectMapper) {
    ...
    fun hentFeed(ctx: Context, pageSize: Int = defaultOutboundPageSize) {
        ...
        if (feedPageItem == null) {
            LOG.warn("Kunne ikke finne feedPageItem - Subject: ${ctx.getSubject()}")
            ctx.status(404)
        } else
    ...
    fun hentFeed(ctx: Context, feedPageId: String, pageSize: Int = defaultOutboundPageSize) {
        LOG.info("Henter feed - Subject: ${ctx.getSubject()} - Side: $feedPageId")
        ...
        if (feed == null) {
            LOG.warn("Kunne ikke finne side - Subject: ${ctx.getSubject()} - Side: $feedPageId")
            ctx.status(404)
        } else if (feed.items.isEmpty()) {
            LOG.warn("Side er tom - Subject: ${ctx.getSubject()} - Side: $feedPageId")
            ctx.status(304)
        } else {
    ...
    fun hentFeedItem(ctx: Context, feedEntryId: String) {
        LOG.info("Henter feed item - Subject: ${ctx.getSubject()} - feedEntryId: $feedEntryId")

        val feed = feedService.hentStillingsAnnonse(UUID.fromString(feedEntryId))

        if (feed == null) {
            LOG.warn("Kunne ikke finne feedItem - Subject: ${ctx.getSubject()} - feedEntryId: $feedEntryId")
            ctx.status(404)
        } else {
